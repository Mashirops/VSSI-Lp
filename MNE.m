function [Kernel,par] = MNE(B,Gain,L,Cortex,InverseMethod,varargin)
...
%       Gain:            Three orientation leadfield matrix (is used to computing the weight of each source for wMNE and LORETA)
...
%               w = 1./sum(Gain.^2,1)';
        ...
        if isempty(OPTIONS.fMRI_prior)
            szl = size(Gain);
            w = squeeze(sum(sum((reshape(Gain,[szl(1) 3 szl(2)/3])) .^2,1),2));
            w = 1 ./ w;
        else
            w = OPTIONS.fMRI_prior;
        end
